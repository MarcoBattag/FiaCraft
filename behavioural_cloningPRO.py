...
# Augmentation: applicata probabilisticamente direttamente su Tensore (GPU)
...
def behavioural_cloning_train(data_dir, in_model, in_weights, out_weights):
    ...
    # Inizializzazione ambiente e agenti su GPU
    ...
    # Freeze dei parametri base e unfreeze selettivo
    ...
    print(f"Inizio training OTTIMIZZATO su dispositivo: {DEVICE}")
    ...
    for batch_i, (batch_images, batch_actions, batch_episode_id, batch_first) in enumerate(data_loader):
        ...
        for image_seq, action_seq, episode_id, first_seq in zip(batch_images, batch_actions, batch_episode_id, batch_first):
            ...
            for t in range(seq_len):
                ...
                # 1. Spostiamo l'immagine su GPU immediatamente (HWC -> CHW)
                img_tensor = th.from_numpy(img_t).to(DEVICE).permute(2, 0, 1).float() / 255.0
                
                # 2. Augmentation direttamente su GPU
                img_tensor = aug_transform(img_tensor)
                
                # 3. Preparazione osservazione per VPT (senza tornare a numpy)
                agent_obs = {"pov": img_tensor.unsqueeze(0)} 
        ...
        if batch_i % LOSS_REPORT_RATE == 0:
            ...
            print(f"Time: {time_since_start:.2f}, Batch: {batch_i}, Loss: {loss_sum / LOSS_REPORT_RATE:.4f}")
            loss_sum = 0
            
            ckpt_path = out_weights.replace(".weights", "_latest.weights")
